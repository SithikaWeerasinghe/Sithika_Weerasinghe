    ...
    <section id="skills" className="section-padding border-t border-white/[0.02]">
      <Container>
        <div className="max-w-3xl mb-16">
          <h2 className="text-4xl md:text-5xl font-bold tracking-tighter mb-4">Technical Proficiency</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12">
          {skillGroups.map((group, i) => (
            <div key={i}>
              <h3 className="text-brand font-mono text-sm tracking-widest uppercase mb-6">{group.title}</h3>
              <ul className="space-y-4">
                {group.skills.map((skill, j) => (
                  <li key={j} className="text-lg font-medium border-l-2 border-white/5 pl-4 hover:border-brand/40 transition-colors">
                    {skill}
                  </li>
                ))}
              </ul>
