    ...
    <section id="projects" className="section-padding bg-black/20">
      <Container>
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-4">
          ...
          <a href="#" className="text-brand font-medium hover:underline flex items-center gap-2">
            View all projects
          </a>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project, i) => (
            <div key={i} className="group glass p-8 rounded-2xl card-hover">
              <p className="text-xs font-mono text-brand mb-4 uppercase tracking-widest">{project.category}</p>
              ...
              <div className="pt-4 border-t border-white/[0.05] group-hover:border-brand/20 transition-colors">
                <span className="text-sm font-medium opacity-50 group-hover:opacity-100 transition-opacity">Explore Project →</span>
              </div>
            </div>
          ))}
        </div>
